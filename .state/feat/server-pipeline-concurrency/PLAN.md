...
- [x] Replace `trackPipeline(promise)` with `runPipeline(fn)` in `pipeline-tracker.ts`
...
- [x] Update call sites to use `runPipeline(fn)` instead of `trackPipeline(promise)`
...
- [x] Add `malformedLineCount` to `NdjsonStream`
...
- [x] Add malformed line warning in `session-pipeline.ts`
