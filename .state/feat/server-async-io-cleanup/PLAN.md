...
- [x] Convert `FsStorageImpl` from `node:fs` sync to `node:fs/promises`
...
- [x] Delete `session-processor.ts` and its test (dead code, M11)
...
|-------|--------|--------|----|-------|
| 1 | complete | `feat/server-async-io-cleanup` | | Async I/O + dead code removed, all existing tests pass |
