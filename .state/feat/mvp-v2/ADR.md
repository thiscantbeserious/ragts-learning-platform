...
- Build with `wasm-pack --target nodejs` for server and `--target web` for future client use

### Consequences

- **Build toolchain**: Rust + wasm-pack added as a build dependency. Developers need Rust installed, or we provide pre-built WASM binaries in CI.
