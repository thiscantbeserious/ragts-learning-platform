...
No external databases, Redis, or Docker containers are needed for development. The project targets Node 20+ (see `.nvmrc` and `engines` in `package.json`); the update script runs `nvm install` to match.
